...
def p_model(p):
	'drh : macros variables states initial GOAL CLN goals const'
	macro = p[1]
	variable = p[2]
	states = p[3]
	init = p[4]
	...
	#print(init)
	#print(goal)
	ha = HA(macro, variable, states, init, goal)
	ha.addConstraints(const)
	p[0] = ha
...
def p_state(p):
	'state : LC MODE NUM INVT CLN invariants FLOW CLN odes JUMP CLN jumps RC'
	mode = p[3]
	invt = p[6]
	flow = p[9]
	jump = p[12]
	p[0] = State(mode, invt, flow, jump)
	#print(invt)
...
def p_error(p):
	...
	while True:      
		tok = lexer1.token() # Get the next token
